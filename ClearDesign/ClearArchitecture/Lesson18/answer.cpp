...
// Добавил методам класса робота константность (неизменямость)
// Теперь они возвращают новый объект с новыми значениями, при этом не изменяя текущее состояние
...
public:
	...
	RobotCleaner move(int dist) const {
		RobotCleaner newCleaner;
		newCleaner.m_dist.x = dist * cos(m_angle);
		newCleaner.m_dist.y = dist * sin(m_angle);
	};
	void turn(int angle) const {
		RobotCleaner newCleaner;
		newCleaner.m_angle = angle;
	};
	void set(CLEANER type) const {
		RobotCleaner newCleaner;
		newCleaner.m_cleaner = type;
	};
	void start() {};
	void stop() {};

	Distance currentDist() const {
		return m_dist;
	}
