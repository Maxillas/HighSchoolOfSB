...
// Сейчас в АПИ хранится объект необходимого робота
// Мы может менять его при инициализации на нужную нам реализацию
// Тем самым мы передаем только одну функцию (объект)
struct RobotState {
    int x = 0;
    int y = 0;
